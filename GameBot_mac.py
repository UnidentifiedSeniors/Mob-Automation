...
class GameBot:
    ...
    def _focus_roblox_window(self) -> bool:
        script = 'tell application "Roblox" to activate'
        try:
            ...
            if result.returncode != 0:
                ...
                self._emit_log(f"AppleScript focus failed: {err}")
                return False
            self._emit_log("Focused Roblox window (AppleScript).")
            time.sleep(0.25)
            return True
        except Exception as exc:
    ...
    def _focus_google_window(self) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                err = (result.stderr or result.stdout or "unknown AppleScript error").strip()
                self._emit_log(f"AppleScript Google focus failed: {err}")
                return False
            self._emit_log("Focused Google Chrome window (AppleScript).")
            time.sleep(0.25)
            return True
        except Exception as exc:
    ...
    def _resolve_screen_point(self, point) -> tuple[int, int]:
        ...
        click_y = int(point.y)
        try:
            # macOS Retina can produce 2x coordinates from screenshot matching.
    ...
    def _get_play_style_region(self):
        # macOS image matching is intentionally full-screen only.
        return None

    def _get_roblox_region_applescript(self):
        script = """
...
""".strip()
    ...
    def _find_and_click_play_button(self) -> bool:
        ...
        # macOS: play button detection is more reliable when scanning the entire screen.
        region = None
